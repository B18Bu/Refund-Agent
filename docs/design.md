...
## 1.1 范围调整记录（2026-08-31）

经工单执行人确认：**当前环境无法部署沙箱，沙箱相关部分整体推迟实现，不纳入本期验收范围。**
推迟范围包括：真实 CubeSandbox 接入、受限 Docker 沙箱适配器、OfficeCLI/Word/Excel 读写、
沙箱逃逸测试及 `docs/evidence/sandbox-escape-report.md`。

推迟期间保留既有沙箱安全护栏（AGENTS.md）：未配置沙箱时必须显式失败、禁止回退宿主机执行、
沙箱实例必须 `try/finally` 销毁；后续沙箱可用时按本方案恢复实施。
...
| 推迟（2026-08-31） | 真实 CubeSandbox、OfficeCLI 读写、沙箱逃逸测试与回收验收 | 沙箱当前无法部署，经执行人确认整体推迟 |
...
5. ~~Word/Excel 的读取、修改和写回只发生在隔离沙箱；至少两类逃逸测试失败，所有实例最终回收。~~（2026-08-31 推迟：沙箱当前无法部署，本期不验收；恢复条件见 1.1）
