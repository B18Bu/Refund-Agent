...
describe('CustomerShell', () => {
  ...
  it('为商城提供紧凑导航、搜索和悬浮客服', () => {
    ...
    expect(screen.getByRole('img', { name: '品牌优选 Logo' })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: '手机数码' })).toHaveAttribute('href', '/shop?category=PHONE')
    expect(screen.getByRole('link', { name: '耳机配件' })).toHaveAttribute('href', '/shop?category=PERIPHERAL')
