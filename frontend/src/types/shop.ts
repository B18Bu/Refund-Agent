...
export type CustomerSupportConversationSummary = { id:number; status:'OPEN'|'IN_PROGRESS'|'RESOLVED'|'NO_CASE'; summary_masked:string|null }
export type CustomerSupportReply = { conversation_id:number; answer:string; intent:string; evidence:{products?:{product_id:number; product_name?:string; description?:string; image_url?:string; category?:ProductCategory; source_url:string}[]; order_id?:number} }
export type CustomerSupportMessage = { id:number; sender:'CUSTOMER'|'ASSISTANT'|'AGENT'|'SYSTEM'; content:string; evidence:Record<string, unknown>; created_at:string|null }
export type CustomerSupportCase = { id:number; conversation_id:number; status:'OPEN'|'IN_PROGRESS'|'RESOLVED'; trigger_reason?:string; summary_masked:string|null; assigned_to:number|null; updated_at?:string|null }
export type CustomerSupportConversationMessages = { status:CustomerSupportCase['status']|'NO_CASE'; messages:CustomerSupportMessage[] }
