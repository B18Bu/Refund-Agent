...
export default function OrderDetail() {
  ...
  const submit = async (value: { order_item_id: number; reason: string; description?: string }) => {
    if (!files.length) { message.error('请至少上传一张商品或问题凭证'); return }
    try {
      const form = new FormData()
      files.forEach((file) => { if (file.originFileObj) form.append('files', file.originFileObj) })
      const upload = await client.post<{ storage_keys: string[] }>('/shop/return-evidence', form)
      await client.post(`/shop/orders/${order.id}/returns`, { ...value, evidence_paths: upload.data.storage_keys }, { headers: { 'X-Idempotency-Key': crypto.randomUUID() } })
