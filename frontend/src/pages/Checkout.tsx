...
export default function Checkout() {
  ...
  return <main className="page-wrap shop-subpage storeflow-page"><p className="shop-eyebrow">CHECKOUT</p><h1>确认订单</h1><p className="storeflow-page__intro">请确认收货信息后提交订单。</p><section className="checkout-panel storeflow-panel"><Typography.Title level={3}>选择收货地址</Typography.Title>{loading ? <p>正在加载地址…</p> : addresses.length ? <Radio.Group className="address-list" value={address} onChange={(event) => setAddress(event.target.value)}>{addresses.map((item) => <Radio aria-label={`${item.recipient_name} ${item.phone} ${item.province}${item.city}${item.district}${item.detail}${item.is_default ? ' 默认地址' : ''}`} className="address-card" value={item.id} key={item.id}><strong>{item.recipient_name} {item.phone}</strong><span>{item.province}{item.city}{item.district}{item.detail}</span>{item.is_default && <em>默认地址</em>}</Radio>)}</Radio.Group> : <Empty description="暂无收货地址" />}</section><Alert className="checkout-notice" type="info" showIcon message="模拟支付说明" description="提交订单后将立即完成模拟支付；本平台不处理真实支付信息，订单金额以服务端返回结果为准。" /><Button type="primary" size="large" block disabled={!address} loading={submitting} onClick={submit}>提交订单并模拟支付</Button></main>
}
