...
from app.commerce_models import Product, ProductVariant, ProductStatus
...
def test_commerce_happy_path(client, db_session):
    ...
    user = User(username="commerce-e2e", password_hash="unused", role=Role.CS)
    db_session.add_all([variant, user]); db_session.commit()
